...
class OTPRepositoryRedis(OTPRepositoryAbstract):
    ...
    def save(self, otp: OTP) -> None:
        serialized_otp = pickle.dumps(otp)
        self.database_session.set(otp.to, serialized_otp)
        self.database_session.expire(otp.to, time=otp.ttl)

    def get(self, to: str) -> Optional[OTP]:
        binary_otp = self.database_session.get(to)
        if binary_otp is None:
            raise OTPDoesNotExistException
        otp: OTP = pickle.loads(binary_otp)
        otp.ttl = self.database_session.ttl(otp.to)
        return otp

    def delete(self, to: str) -> None:
        self.database_session.delete(to)
